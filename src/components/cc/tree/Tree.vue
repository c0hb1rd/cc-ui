<template>
    <div class="cc-tree">
        <div class="cc-tree-node" v-for="item in data" v-if="item.show !== undefined ? item.show : true ">
            <div class="cc-tree-node__content flex-row flex-y-center"
                 :ref="`cc-node-content-${item.id}`"
                 @click.stop="choose(`cc-node-content-${item.id}`)"
                 :style="{paddingLeft: padding + 'px'}">
                <div :ref="`cc-node-icon-${item.id}`"
                     @click="toggle(`cc-node-icon-${item.id}`, `cc-node-${item.id}`, item)"
                     class="cc-tree-node__icon flex-row flex-a-center"><i
                        v-if="!item.disableExpand && item.children && item.children.length > 0"
                        class="fa fa-caret-right"></i></div>
                <item-node :props="props" :node="item" :itemNode="item.itemNode || itemNode"></item-node>
            </div>
            <div class="cc-tree-node__children" data-height="" @click.native.prevent="() => {}"
                 :ref="`cc-node-${item.id}`" v-if="item[props.children] && item[props.children].length > 0">
...
<script>
    ...
    export default {
            ...
            itemNode: {
                default: function () {
                    ...
                    return {
                        component: TreeNode,
                        on: {
                            nodeClick: function (event, val) {
                                self.$emit("nodeClick", event, val);
                            }
                        }
                    }
                }
        ...
        methods: {
            ...
            choose(content) {
                this.$(".cc-tree-node__content").removeClass("choose-node");
                this.$refs[content][0].className += " choose-node";
            },
